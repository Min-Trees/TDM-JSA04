...
async function loadProducts() {
    try {
        ...
        const savedProducts = localStorage.getItem('products_candy');
        if (savedProducts) {
            const products = JSON.parse(savedProducts);
            displayProducts(products, "candy-grid");
            return;
        }

        // Nếu không có, load từ JSON như cũ
        const response = await fetch("data/candy.json");
        const data = await response.json();
        displayProducts(data.Candy, "candy-grid");
    } catch (error) {
...
function createProductCard(product) {
    ...
    card.addEventListener("click", () => {
        ...
        window.location.href = `product.html?id=${product.id}&category=candy`;
    });
    ...
    card.addEventListener("mouseleave", () => {
        // Dừng tự động chuyển và quay về ảnh đầu tiên
