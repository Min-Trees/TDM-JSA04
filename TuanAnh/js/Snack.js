...
async function loadProducts() {
    try {
        const savedProducts = localStorage.getItem('products_snack');
        if (savedProducts) {
            const products = JSON.parse(savedProducts);
            displayProducts(products, "snack-grid");
            return;
        }

        const response = await fetch("data/snack.json");
        const data = await response.json();
        displayProducts(data.Snack, "snack-grid");
    } catch (error) {
...
function createProductCard(product) {
    ...
    card.addEventListener("click", () => {
        ...
        window.location.href = `product.html?id=${product.id}&category=snack`;
    });
